...
{
    const FIELDS_EXTRA_CLASS_PREFIX = 'Saurav-cpcl-';

    const FIELD_TEMPLATE = 'Saurav_CmsLinkToMenu::system/config/form/field/array.phtml';

    private $linkTypesRenderer;

    private $cmsPagesRenderer;
    /**
     * @var \Saurav\CmsLinkToMenu\Model\Config\Source\Pages
     */
    private $cmsPages;

    public function __construct(
        \Magento\Backend\Block\Template\Context $context,
        \Saurav\CmsLinkToMenu\Model\Config\Source\Pages $cmsPages,
        array $data = []
    ) {
    ...
    private function getCmsPagesRenderer()
    {
        if (!$this->cmsPagesRenderer) {
            $this->cmsPagesRenderer = $this->getLayout()->createBlock(
                \Saurav\CmsLinkToMenu\Block\Adminhtml\Form\Field\Pages::class,
                '',
    ...
    private function getLinkTypesRenderer()
    {
        if (!$this->linkTypesRenderer) {
            $this->linkTypesRenderer = $this->getLayout()->createBlock(
                \Saurav\CmsLinkToMenu\Block\Adminhtml\Form\Field\LinkTypes::class,
                '',
                ['data' => ['is_render_to_js_template' => true]]
            );
